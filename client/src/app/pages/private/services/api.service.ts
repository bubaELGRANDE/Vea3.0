...
import { routes } from '../../../core/api/endpoints';
...
export class ApiService {
  ...
  getUserById(id: string | number) {
    return this.get(routes.getUserById, { id });
  }

  createUser(userData: any) {
    return this.post(routes.getUser, userData); // Usar el mismo endpoint base para crear
  }
  ...
  deleteUser(id: string | number) {
    return this.delete(routes.deleteUser, { id });
  }

  // Métodos adicionales de usuario que podrían necesitarse
  updateProfile(profileData: any) {
    return this.put(routes.updateUser, profileData, { id: profileData.id });
  }
  changePassword(passwordData: { currentPassword: string; newPassword: string; userId: string | number }) {
    // Asumiendo que existe un endpoint específico para cambio de contraseña
    // Si no existe, usar el endpoint genérico de usuario
    return this.put(routes.updateUser, passwordData, { id: passwordData.userId });
  }
